        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', gap: '12px', position: 'relative' }}>
            ...
            <input
              ...
              placeholder="Search description, exact reporter ID, or resource... (Press Enter)"
              style={{ 
                flex: 1, 
            ...
            />
            <button className="btn-primary" onClick={applyFilters} style={{ padding: '0 32px', borderRadius: '14px', whiteSpace: 'nowrap' }}>
              Search Text
            </button>
          </div>
          ...
          <div style={{ 
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ fontSize: '0.8rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                <Filter size={14} style={{ display: 'inline', marginRight: '6px', verticalAlign: '-3px' }} />
                Instant Category Filters
              </div>
              {(draftFilters.status || draftFilters.priority || draftFilters.category || draftFilters.assignee || draftFilters.slaState) && (
                <button onClick={resetFilters} style={{ background: 'var(--danger-muted)', border: 'none', color: 'var(--danger)', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', padding: '6px 12px', borderRadius: '100px', transition: 'all 0.2s' }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
                  <X size={14} strokeWidth={2.5} /> Reset All Filters
                </button>
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '12px' }}>
              <select 
                ...
                onChange={(event) => { updateDraft('status', event.target.value); setFilters(prev => ({...prev, status: event.target.value})); setPage(0); }} 
                className="filter-select"
                style={draftFilters.status ? { color: 'var(--accent-base)', borderColor: 'var(--accent-muted)', background: 'var(--bg-surface)' } : {}}
              >
              ...
              <select 
                ...
                onChange={(event) => { updateDraft('priority', event.target.value); setFilters(prev => ({...prev, priority: event.target.value})); setPage(0); }} 
                className="filter-select"
                style={draftFilters.priority ? { color: 'var(--accent-base)', borderColor: 'var(--accent-muted)', background: 'var(--bg-surface)' } : {}}
              >
              ...
              <select 
                ...
                onChange={(event) => { updateDraft('category', event.target.value); setFilters(prev => ({...prev, category: event.target.value})); setPage(0); }} 
                className="filter-select"
                style={draftFilters.category ? { color: 'var(--accent-base)', borderColor: 'var(--accent-muted)', background: 'var(--bg-surface)' } : {}}
              >
          ...
          <style>{`
            ...
            .filter-select:hover {
              ...
              box-shadow: 0 4px 12px rgba(42, 20, 180, 0.05);
            }
          `}</style>
        </div>
      )}
